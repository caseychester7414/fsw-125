...
app.delete("/tweets/:tweetsId", (req, res) => {
    const tweetsId = req.params.tweetsId
    const tweetsIndex = tweets.findIndex(tweets => tweets.id === tweetsId)
    tweets.splice(tweetsIndex, 1)
    res.send("Successful deleted tweet!")
})
app.put("/tweets/:tweetsId", (req, res) => {
    console.log(req.body)
    const tweetsId = req.params.tweetsId
    console.log(tweetsId)
    const tweetsIndex = tweets.findIndex(tweets => { tweets.id == tweetsId})
    const updatedTweet = Object.assign(tweets[tweetsIndex], req.body);
